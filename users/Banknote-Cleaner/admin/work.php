...
<div class="wrapper">
                ...
                <div class="box-body">
                    ...
                    <form name="request_form" method="post">
                      <div class="control-group form-group">
                          ...
                          <input type="text" class="form-control" id="idnumber" name="idnumber" value="<?php echo $idnumber; ?>" readonly>
                      </div>
                      <div class="control-group form-group">
                          <label>ID:</label>
                          <input type="text" class="form-control" id="no" name="no" value="<?php echo htmlspecialchars($No); ?>" readonly>
                      </div>

                      <div class="control-group form-group">
                        <label>Name:</label>
                        <input type="text" class="form-control" id="fname" name="fname" value="<?php echo $fname . ' ' . $lname; ?>" readonly>
                      </div>
    ...
    <?php
    $qry = "SELECT id, tool_name FROM tools"; // Fetch both id and tool_id
    $result = $conn->query($qry);
    $num = $result->num_rows;
    if ($num > 0) {
        echo '<select required name="tool_id" class="form-control mb-1">'; // Use tool_id as the name
        echo '<option value="">Select Material</option>';
        while ($rows = $result->fetch_assoc()) {
            echo '<option value="' . $rows['id'] . '">' . $rows['tool_name'] . '</option>'; // Pass id as value
        }
        echo '</select>';
    } else {
