...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['action']) && $_POST['action'] == 'apply') {
    ...
    if (empty($supplierFname) || empty($supplierLname) || empty($supplierPhone) || empty($supplierEmail) || empty($unitPrice) || empty($amount)) {
        ...
        header("Location: product_tenders.php"); // Redirect to avoid resubmission
        exit();
    }
    ...
    $stmt = $conn->prepare("SELECT COUNT(*) AS applied FROM product_tenders WHERE id = ? AND fname = ? AND lname = ? AND phone = ? AND email = ?");
    ...
    if ($row['applied'] > 0) {
        $_SESSION['error'] = "You have already applied for this request.";
        header("Location: product_tenders.php");
        exit();
    }

    // Update the product_tenders table to mark that this supplier has applied and update unit_price and amount
    $stmt = $conn->prepare("UPDATE product_tenders SET fname = ?, lname = ?, phone = ?, email = ?, unit_price = ?, Amount = ?, applied = 1 WHERE id = ?");
    if (!$stmt) {
    ...
    <div class="wrapper">
        ...
        <div class="content-wrapper">
          ...
          ";
                ...
                $sql = "SELECT * FROM productionmanager WHERE fname = ?";
                ...
                // Fetch product_tenders
                $sql = "SELECT * FROM product_tenders WHERE deadline > NOW() ORDER BY posted_at DESC";
        ...
        <div class='card' style='background-color: white;'>
          <div class='card-header'>
            ...
            <div class='card-tools'>
              <input type='text' id='search-bar' class='form-control' placeholder='Search Product Requests'>
            </div>
          </div>
          <div class='card-body'>
        ";
                ...
                if ($result->num_rows > 0) {
                    ...
                    echo "<thead><tr><th>Title</th><th>Description</th><th>Quantity</th><th>Posted At</th><th>Deadline</th><th>Status</th><th>Actions</th></tr></thead>";
                    echo "<tbody>";
                    ...
                    while ($row = $result->fetch_assoc()) {
                        ...
                        } else if ($row['supplier_status'] == 0) {
                            echo "<span class='label label-info'>Applied</span>";
                        } else if ($row['supplier_status'] == 1) {
                ...
                <button class='btn btn-primary apply-btn'" . ($isPastDeadline || $hasApplied ? " disabled" : "") . " data-id='" . $row['id'] . "' data-title='" . htmlspecialchars($row['title']) . "' data-description='" . htmlspecialchars($row['description']) . "' data-quantity='" . htmlspecialchars($row['quantity']) . "' data-unit_price='" . htmlspecialchars($row['unit_price']) . "' data-amount='" . htmlspecialchars($row['Amount']) . "' data-deadline='" . htmlspecialchars($row['deadline']) . "'>Apply</button>
                    ...
                    echo "<p>No product requests available.</p>";
                }
        ...
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title" id="applyModalLabel">Request Application</h5>
                    ...
                    <form id="apply-form" action="" method="POST">
                        ...
                        <div class="form-group">
                            ...
                            <input type="number" step="0.01" class="form-control" name="unit_price"
                                id="modal-unit_price" required min="50" max="150000"
                                oninput="if(this.value > 150000) { alert('Maximum unit price allowed is 150000'); this.focus(); }"
                                required>
                        </div>
                        <div class="form-group">
                            <label for="modal-amount">Amount</label>
                            <input type="number" step="0.01" class="form-control" name="amount" id="modal-amount"
                                required>
                        </div>
                        <div class="form-group">
                        ...
                        <div class="form-group">
                            ...
                            <input type="text" class="form-control" name="supplier_fname" id="supplier_fname"
                                value="<?php echo htmlspecialchars($user['fname']); ?>" readonly>
                        </div>
                        <div class="form-group">
                            <label for="supplier_lname">Last Name</label>
                            <input type="text" class="form-control" name="supplier_lname" id="supplier_lname"
