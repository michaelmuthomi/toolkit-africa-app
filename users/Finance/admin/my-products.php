...
<body class="hold-transition skin-blue sidebar-mini">
                        ...
                        <tr>
                          ...
                          <td>
                            <?php if ($row['payment_status'] == 1) : ?>
                              <button class="btn btn-primary" style="border: 1px solid white; background-color: #5cb85c;" onclick="printReceipt('<?php echo $row['idnumber']; ?>', '<?php echo addslashes(nl2br($row['product_names'])); ?>', '<?php echo addslashes(nl2br($row['prices'])); ?>', '<?php echo addslashes(nl2br($row['quantities'])); ?>', '<?php echo addslashes(nl2br($row['totals'])); ?>', '<?php echo $row['totalamount']; ?>', '<?php echo $row['transactioncode']; ?>', '<?php echo $row['date_paid']; ?>')">Receipt</button>
                            <?php endif; ?>
                          </td>
                        </tr>
                      <?php endwhile; ?>
  ...
  <script>
    ...
    function printReceipt(orderId, productNames, prices, quantities, totals, totalAmount, transactionCode, datePaid) {
      ...
      console.log("printReceipt called with:", {
        orderId, productNames, prices, quantities, totals, totalAmount, transactionCode, datePaid
      });
