<?php
	...
	if(isset($_POST['login'])){
		...
		if($query->num_rows < 1){
			...
			{
			$_SESSION['admin'] = $row['idnumber'];
			$_SESSION['user_id'] = $row['idnumber']; // Keep for compatibility if used elsewhere
			$_SESSION['name'] = $row['fname']; // Store name separately for display fallback
