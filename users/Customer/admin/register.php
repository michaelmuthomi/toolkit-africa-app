...
// Fetch customer data from the database
$sql = "SELECT idnumber, fname, lname, email, phone, address FROM customer WHERE idnumber = :idnumber";
$query = $dbh->prepare($sql);
$query->bindParam(':idnumber', $userid, PDO::PARAM_STR);
$query->execute();
$customer = $query->fetch(PDO::FETCH_ASSOC);

// Debugging session value
if (!$customer) {
    $_SESSION['error'] = "Error fetching customer details. Please re-login.";
...
if (isset($_POST['send'])) {
    ...
    if (!preg_match($naming, $transactioncode)) {
        ...
        $sql = "INSERT INTO booking (idnumber, fname, lname, email, phone, address, service, charges, transactioncode, payment_status, supervisor_status) 
                VALUES (:idnumber, :fname, :lname, :email, :phone, :address, :service, :charges, :transactioncode, '0', '0')";
        $query = $dbh->prepare($sql);
