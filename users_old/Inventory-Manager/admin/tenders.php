...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['action']) && $_POST['action'] == 'post') {
    ...
    if (empty($title) || empty($description) || empty($quantity) || empty($deadline)) {
        ...
        header('Location: tenders.php'); // Redirect to the form page
        exit();
    }
    ...
    $stmt = $conn->prepare("INSERT INTO tenders (title, description, quantity, deadline) VALUES (?, ?, ?, ?)");
    $stmt->bind_param('ssis', $title, $description, $quantity, $deadline);

    if ($stmt->execute()) {
        $_SESSION['success'] = 'Tender posted successfully.';
    } else {
        $_SESSION['error'] = 'Failed to post tender.';
    }
...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['action']) && $_POST['action'] == 'delete') {
    ...
    // Prepare and execute SQL query
    $stmt = $conn->prepare("DELETE FROM tenders WHERE id=?");
    $stmt->bind_param('i', $id);

    if ($stmt->execute()) {
        $_SESSION['success'] = 'Tender deleted successfully.';
    } else {
    ...
    header('Location: tenders.php'); // Redirect to the list page
    exit();
}
...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['action']) && $_POST['action'] == 'update') {
    ...
    // Prepare and execute SQL query
    $stmt = $conn->prepare("UPDATE tenders SET title=?, description=?, quantity=?, unit_price=?, deadline=?, amount=? WHERE id=?");
    $stmt->bind_param('ssidsii', $title, $description, $quantity, $unit_price, $deadline, $amount, $id);

    if ($stmt->execute()) {
        $_SESSION['success'] = 'Tender updated successfully.';
    } else {
        $_SESSION['error'] = 'Failed to update tender.';
    }
  ...
  <div class="content-wrapper">
    ...
    <section class="content">
      ...
      <div class="box">
        <div class="box-header">
          <h3 class="box-title">Post a New Tender</h3>
        </div>
        <div class="box-body">
    ...
    <div class="form-group row mb-3">
        <div class="col-xl-6">
            ...
            $qry = "SELECT tool_name FROM tools";
            ...
            if ($result && $result->num_rows > 0) {
                ...
                while ($rows = $result->fetch_assoc()) {
                    echo '<option value="' . htmlspecialchars($rows['tool_name']) . '">' . htmlspecialchars($rows['tool_name']) . '</option>';
                }
                echo '</select>';
            } else {
    ...
    <button type="submit" class="btn btn-primary">Post Tender</button>
      ...
      <div class="box">
        <div class="box-header">
            <h3 class="box-title">Posted Tenders</h3>
        </div>
        <div class="box-body">
        ...
        <div class="modal-content">
          ...
          <div class="modal-body">
            <form id="editTenderForm" action="" method="POST">
              ...
              <div class="form-group">
                <label for="edit_title">Tender Title</label>
