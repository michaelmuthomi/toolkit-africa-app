...
if ($cadm_password!= $password) {
    ...
    if(mysqli_num_rows(mysqli_query($conn,"SELECT * from inventorymanager where email='$email'"))>0){
        if(mysqli_query($conn,"UPDATE inventorymanager set password='$password' where email='$email'")){
