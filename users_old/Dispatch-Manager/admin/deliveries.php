...
$sql = "SELECT * FROM dispatch_tasks where dispatch= '$dispatch'"; // Fetch all dispatch tasks
...
if (isset($_GET['id']) && is_numeric($_GET['id'])) {
    ...
    $query = "UPDATE dispatch_tasks SET status = 2 WHERE No = $task_id";

    if (mysqli_query($conn, $query)) {
        $_SESSION['success'] = "Delivery Completed- waiting for customer Approval.";
    } else {
            ...
            <div class="box">
              <div class="box-body">
                <div style="overflow-x: auto;">
                  ...
                  echo "<td><button class='btn btn-primary' data-toggle='modal' data-target='#confirmModal{$row['No']}' {$buttonDisabled}>Mark Delivery Completed</button></td>";
                       ?>
                            <?php // Modal for each task
                                            ...
                                            echo "<h4 class='modal-title' id='confirmModalLabel{$row['No']}'>Confirm Mark Delivered</h4>";
                                            ...
                                            echo "<a href='deliveries.php?id={$row['No']}' class='btn btn-primary'>Confirm</a>";
                                            echo "</div>";
                                            echo "</div>";
                                            echo "</div>";
                                            echo "</div>";?>
