...
if ($cadm_password!= $password) {
    ...
    if(mysqli_num_rows(mysqli_query($conn,"SELECT * from dispatchmanager where email='$email'"))>0){
        if(mysqli_query($conn,"UPDATE dispatchmanager set password='$password' where email='$email'")){
